// ============================================
// SUPPLIER DASHBOARD - DRAG-TO-REORDER EDIT MODE
// Same mechanic as the Store Manager/HR dashboards (see
// store_manager/dashboard-layout.js): each .dash-canvas-row is an
// independent sortable zone (widgets never move between rows, only within
// their own row). The resulting order is saved per account via
// api_save_supplier_dashboard_layout and restored on load via
// api_get_supplier_dashboard_layout.
// ============================================
...
(function () {
    ...
    function collectAllOrders() {
        return {
            stats: getRowOrder(document.getElementById('spDashCanvasStats')),
            content: getRowOrder(document.getElementById('spDashCanvasContent')),
        };
    }

    function saveLayout() {
        fetch('?page=api_save_supplier_dashboard_layout', {
            method: 'POST',
    ...
    function loadLayout() {
        fetch('?page=api_get_supplier_dashboard_layout')
            .then(function (r) { return r.json(); })
            .then(function (data) {
                var order = data.success && data.data ? data.data.widget_order : null;
                if (!order) return;
                applyOrder(document.getElementById('spDashCanvasStats'), order.stats);
                applyOrder(document.getElementById('spDashCanvasContent'), order.content);
            })
            .catch(function (err) {
    ...
    function showRevertConfirm() {
        ...
        function finish(keep) {
            ...
            if (!keep) {
                applyOrder(document.getElementById('spDashCanvasStats'), preEditSnapshot.stats);
                applyOrder(document.getElementById('spDashCanvasContent'), preEditSnapshot.content);
            }
