// ============================================
// OWNER - PRODUCT PROPOSALS
// ============================================

let ppProposals = [];
let ppDecidingId = null;
...
function renderProposals() {
    ...
    // Awaiting-owner items surface first -- that's the actionable queue.
    const sorted = [...ppProposals].sort((a, b) => {
        if (a.status === 'pending_owner' && b.status !== 'pending_owner') return -1;
        if (b.status === 'pending_owner' && a.status !== 'pending_owner') return 1;
...
function openProposalDetail(id) {
    ...
    if (p.status === 'rejected' && p.owner_decided_at) {
        html += `<div class="sm-section-divider"></div><h6 class="sm-section-title"><i class="bi bi-person-check"></i> Your Decision</h6><p class="text-danger small mb-0">Rejected: ${ppEscapeHtml(p.owner_reject_reason || 'No reason given.')}</p>`;
    ...
    if (p.status === 'pending_owner') {
        footer.innerHTML = `
            ...
            <button type="button" class="btn btn-danger btn-sm" id="rejectProposalBtn">Reject</button>
            <button type="button" class="btn btn-success btn-sm" id="approveProposalBtn"><i class="bi bi-check-circle"></i> Approve</button>
        `;
        document.getElementById('rejectProposalBtn').addEventListener('click', () => openRejectModal(p.id));
        document.getElementById('approveProposalBtn').addEventListener('click', () => decideProposal(p.id, 'approve'));
    } else {
...
document.getElementById('confirmRejectBtn')?.addEventListener('click', function () {
    ...
    if (!reason) {
        Swal.fire('Reason required', 'Please explain why this is being rejected.', 'warning');
        return;
    }
    ...
    decideProposal(ppDecidingId, 'reject', reason);
});

function decideProposal(id, action, reason) {
    fetch('?page=api_owner_decide_product_proposal', {
        method: 'POST',
        ...
        .then(res => {
            if (res.success) {
                bootstrap.Modal.getInstance(document.getElementById('proposalDetailModal'))?.hide();
                Swal.fire({ icon: 'success', title: action === 'approve' ? 'Approved!' : 'Rejected', text: res.message, timer: 1800, showConfirmButton: false });
                loadProposals();
            } else {
                Swal.fire('Error', res.message || 'Failed to save decision', 'error');
            }
        })
